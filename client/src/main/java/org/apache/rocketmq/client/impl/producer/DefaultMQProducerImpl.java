...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                // 2. 构建MQClientInstance
                this.mQClientFactory = MQClientManager.getInstance().getOrCreateMQClientInstance(this.defaultMQProducer, rpcHook);
                // 3. 注册defaultMQProducer
     ...
     * @param sendCallback      异步发送时的callback
     * @param timeout           超时时间
     * @return 发送结果
     */
    private SendResult sendDefaultImpl(
        ...
        // 1. 发送前的检查
        this.makeSureStateOK();
        //    这里的checkMessage和DefaultMQProducer.send方法中的重复了
        Validators.checkMessage(msg, this.defaultMQProducer);
        final long invokeID = random.nextLong();
        long beginTimestampFirst = System.currentTimeMillis();
        long beginTimestampPrev = beginTimestampFirst;
        long endTimestamp = beginTimestampFirst;
        // 2. 获取Topic路由信息(若topicPublishInfoTable缓存中存在对应有效记录，则直接返回，否则查询NameServer)
        TopicPublishInfo topicPublishInfo = this.tryToFindTopicPublishInfo(msg.getTopic());
        // 3. 若路由信息有效，则开始执行send逻辑
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            ...
            // 从NameServer处查询路由信息，并更新路由缓存(topicPublishInfoTable)
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        switch (sendResult.getSendStatus()) {
            ...
            case SEND_OK: {
                try {
                    ...
                    // 判断是否指定了LocalTransactionExecuter，5.0.0后统一使用TransactionListener
            ...
            break;
            // 3.2 若遇到其他各种原因的失败，则不执行本地事务，将本次事务状态设为ROLLBACK_MESSAGE
        ...
        /*
           4. 根据上面得到的事务状态向Broker发请求以执行"commit/rollback/告知本地事务状态未知"
              ------------------------------------------------------------------------------
              该请求以OneWay通信模式发送
         */
        try {
